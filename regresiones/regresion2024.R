...
df_cox2024[comorbilidades] <- lapply(df_cox2024[comorbilidades], factor)
...
modelo_cox <- coxph(formula_cox, data = df_cox2024)
...
ggsurvplot(survfit(Surv(DIAS_SINTOMAS_DEF, DEFUNCION) ~ DIABETES, data = df_cox2024),
           data = df_cox2024,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = TRUE,
...
ggsurvplot(survfit(Surv(DIAS_SINTOMAS_DEF, DEFUNCION) ~ HIPERTENSION, data = df_cox2024),
           data = df_cox2024,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = TRUE,
...
ggsurvplot(survfit(Surv(DIAS_SINTOMAS_DEF, DEFUNCION) ~ OBESIDAD, data = df_cox2024),
           data = df_cox2024,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = TRUE,
...
CreateTableOne(vars = c("DIABETES", "HIPERTENSION", "TABAQUISMO", "RENAL_CRONICA"),
               strata = "OBESIDAD", data = df_cox2024, test = TRUE)
