...
comorbilidades <- c('NEUMONIA', 'DIABETES', 'EPOC', 'ASMA', 'INMUSUPR',
                    'HIPERTENSION', 'CARDIOVASCULAR', 'RENAL_CRONICA', 'TABAQUISMO')
...
# Definir la fórmula del modelo de Cox
formula_cox <- as.formula("Surv(DIAS_SINTOMAS_DEF, DEFUNCION) ~ NEUMONIA + DIABETES + EPOC + INMUSUPR + HIPERTENSION + 
...
modelo_cox_estrat <- coxph(Surv(DIAS_SINTOMAS_DEF, DEFUNCION) ~ 
                             ...
                             INMUSUPR + strata(HIPERTENSION) + 
                             CARDIOVASCULAR + OBESIDAD + RENAL_CRONICA + strata(TABAQUISMO),
                           data = df_cox2023)
# Ver el resumen del modelo
...
ggsurvplot(survfit(Surv(DIAS_SINTOMAS_DEF, DEFUNCION) ~ OBESIDAD, data = df_cox2023),
           data = df_cox2023,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = TRUE,
...
ggsurvplot(survfit(Surv(DIAS_SINTOMAS_DEF, DEFUNCION) ~ HIPERTENSION, data = df_cox2023),
           data = df_cox2023,
           risk.table = TRUE,
           pval = TRUE,
           conf.int = TRUE,
...
CreateTableOne(vars = c("DIABETES", "HIPERTENSION", "TABAQUISMO", "RENAL_CRONICA"),
               strata = "OBESIDAD", data = df_cox2023, test = TRUE)
